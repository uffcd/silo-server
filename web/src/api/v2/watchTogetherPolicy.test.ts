...
it("uses returned policy and sends exactly one PATCH without guest proof", async () => {
  ...
  await act(async () => {
    await setWatchTogetherGuestControl(result.current.updatePolicy, "guest_play_pause");
  });
  expect(fetch).toHaveBeenCalledTimes(1);
  expect(fetch.mock.calls[0]![0]).toBe("/api/v2/watch-together/rooms/room/policy");
...
it.each([200, 500])("refuses old authority and old-room %s receipts", async (status) => {
  ...
  const old = result.current.updatePolicy;
  setProfileToken("replacement");
  rerender({ id: "room" });
  await act(async () => {
    await expect(old("host_only")).rejects.toThrow();
  });
  expect(fetch).not.toHaveBeenCalled();
  let done!: Promise<void>;
  act(() => {
    done = setWatchTogetherGuestControl(result.current.updatePolicy, "guest_play_pause");
  });
  rerender({ id: "other" });
  await act(async () => {
...
it.each(["room", "other"])(
  "compares policy generations only within room %s",
  async (targetRoom) => {
    ...
    const fetch = vi.fn().mockImplementation((url: string) => {
      ...
      if (url.endsWith("/policy")) return Promise.resolve(roomResponse(targetRoom, 2));
    ...
    await act(async () => {
      const receipt = await result.current.updatePolicy("host_only");
