...
import { useAutoscanEvents } from "../useAutoscan";
const row = {
  id: "7",
  source_id: null,
  plugin_id: "synthetic",
...
it("follows signed pages to the requested numbered page and adapts IDs", async () => {
  const fetchMock = vi
    ...
    .mockResolvedValueOnce(response([{ ...row, id: "8" }], "", 2));
...
it.each([null, "pin-b"])("isolates cached success on PIN transition %s", async (pin) => {
  ...
  vi.stubGlobal("fetch", fetchMock);
  const { result, rerender } = renderHook(() => useAutoscanEvents(), fixture());
  await waitFor(() => expect(result.current.isSuccess).toBe(true));
...
it("rejects late decoded rows after authority replacement", async () => {
  ...
  const f = fixture();
  const { result } = renderHook(() => useAutoscanEvents(), f);
