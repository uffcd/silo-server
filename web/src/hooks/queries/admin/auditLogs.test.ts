...
it("adopts filters, full scoped continuation and checked UI identifiers", async () => {
  ...
  expect(url.pathname).toBe("/api/v2/admin/logs/audit");
  expect(url.searchParams.get("method")).toBe("GET");
  expect(url.searchParams.get("status_code")).toBe("200");
  expect(url.searchParams.get("client_ip")).toBe("192.0.2.7");
  ...
  expect(result.current.data).toMatchObject({
    entries: [{ id: 42, user_id: 7, impersonator_user_id: 9 }],
...
it("rejects a read decoded after the acting profile changes", async () => {
  ...
  const { result } = renderHook(() => useAuditLogs({ limit: 12 }), { wrapper });
...
it("rejects identifiers the current UI cannot represent rather than rounding", async () => {
  ...
  const { result } = renderHook(() => useAuditLogs({}), fixture());
...
it("does not read without an acting profile", () => {
  ...
  vi.stubGlobal("fetch", fetchMock);
  renderHook(() => useAuditLogs({}), fixture());
