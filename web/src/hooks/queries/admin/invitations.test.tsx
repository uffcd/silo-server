...
import { invitationScope, captureInvitationAuthority } from "@/api/v2/invitations";
import { adminKeys } from "../keys";
import { useAdminInvitations, useCreateInvitation } from "./invitations";
const key = {
  id: "1",
  user_id: "2",
  label: "Tool",
...
it("loads explicitly, refetches ordinary two-page results, and restarts a failed cursor", async () => {
  ...
  const { result } = renderHook(() => useAdminInvitations(), { wrapper });
  ...
  malformed = true;
  await act(async () => {
    await expect(result.current.restart()).rejects.toThrow();
  });
...
it("does not cache creation secrets and invalidates only captured list scope", async () => {
  const fetch = vi.fn<typeof globalThis.fetch>(async () =>
    json({
      invitation: key,
      claim_url: "https://example.invalid/invite/creation-secret",
  ...
  const target = [...adminKeys.invitations(), invitationScope(context)];
  const other = [...adminKeys.invitations(), "another-profile"];
  client.setQueryData(target, { items: [key] });
  client.setQueryData(other, { items: [key] });
  const { result, unmount } = renderHook(() => useCreateInvitation(), { wrapper });
  await act(async () => {
