...
import { useDeletePluginRepository } from "./plugins";
...
it("captures the deletion target before offline queueing and sends once", async () => {
  ...
  const { result } = renderHook(useDeletePluginRepository, fixture());
  let id = 7;
...
it("does not invalidate another authority after late deletion acknowledgement", async () => {
  ...
  const { result } = renderHook(useDeletePluginRepository, { wrapper });
  act(() => result.current.mutate(7));
