...
import { useRotateServerNotificationChannelSecret } from "./serverNotificationChannels";
...
it("invalidates only the captured server channel cache and rejects a stale receipt", async () => {
  ...
  const { result } = renderHook(() => useRotateServerNotificationChannelSecret(), {
...
it("sends exact server channel rotation once on failure", async () => {
  const client = new QueryClient();
  const { result } = renderHook(() => useRotateServerNotificationChannelSecret(), {
    wrapper: ({ children }: { children: ReactNode }) =>
      createElement(QueryClientProvider, { client }, children),
  ...
  for (const status of [401, 403, 500]) {
    ...
    expect(String(fetch.mock.calls[0]![0])).toContain(
      "/api/v2/admin/notifications/server-channels/row-one/rotate-secret",
    );
...
it("refuses a paused rotation after authority replacement", async () => {
  ...
  const fetch = vi.fn();
  vi.stubGlobal("fetch", fetch);
  const { result, rerender } = renderHook(() => useRotateServerNotificationChannelSecret(), {
    wrapper: ({ children }: { children: ReactNode }) =>
      createElement(QueryClientProvider, { client }, children),
  });
