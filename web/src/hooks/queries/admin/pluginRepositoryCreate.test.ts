...
import { useCreatePluginRepository } from "./plugins";
const body = {
...
it("copies submit body before offline queueing and sends one v2 creation", async () => {
  ...
  draft.display_name = "Later draft";
  act(() => onlineManager.setOnline(true));
  await waitFor(() => expect(result.current.isSuccess).toBe(true));
  expect(fetchMock).toHaveBeenCalledOnce();
  expect(String(fetchMock.mock.calls[0]?.[0])).toContain("/api/v2/admin/plugins/repositories");
  const init = fetchMock.mock.calls[0]?.[1] as RequestInit;
  expect(init.method).toBe("POST");
  expect(JSON.parse(String(init.body))).toEqual(body);
});
it("refuses a queued create after authority replacement", async () => {
  onlineManager.setOnline(false);
...
it.each(["401", "network"])(
  "never replays uncertain %s creation under global retry3",
  async (failure) => {
    const fetchMock =
      failure === "network"
...
it("does not invalidate the new authority after late creation acknowledgement", async () => {
  ...
  const { result } = renderHook(useCreatePluginRepository, { wrapper });
  act(() => result.current.mutate(body));
