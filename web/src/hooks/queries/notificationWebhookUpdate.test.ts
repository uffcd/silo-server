...
import { useUpdateNotificationWebhook } from "./notificationWebhooks";
...
it("invalidates only the captured webhook cache and rejects a stale receipt", async () => {
  ...
  const { result } = renderHook(() => useUpdateNotificationWebhook(), {
    wrapper: ({ children }: { children: ReactNode }) =>
      createElement(QueryClientProvider, { client }, children),
  });
  await act(async () => {
    await result.current.mutateAsync({ id: "one", etag: '"original"', name: "updated" });
  });
  ...
  await act(async () => {
    await expect(
      result.current.mutateAsync({ id: "two", etag: '"original"', name: "updated" }),
    ).rejects.toThrow();
  });
...
it("sends exact webhook update once on failure", async () => {
  const client = new QueryClient();
  const { result } = renderHook(() => useUpdateNotificationWebhook(), {
    wrapper: ({ children }: { children: ReactNode }) =>
      createElement(QueryClientProvider, { client }, children),
  ...
  for (const status of [401, 403, 412, 428, 500]) {
    ...
    await act(async () => {
      await expect(
        result.current.mutateAsync({ id: "row-one", etag: '"original"', name: "updated" }),
      ).rejects.toThrow();
    });
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(String(fetch.mock.calls[0]![0])).toContain("/api/v2/notifications/webhooks/row-one");
    expect(fetch.mock.calls[0]![1].method).toBe("PUT");
    expect(new Headers(fetch.mock.calls[0]![1].headers).get("If-Match")).toBe('"original"');
  }
  client.clear();
});
...
it("refuses a paused update after authority replacement", async () => {
  ...
  const fetch = vi.fn();
  vi.stubGlobal("fetch", fetch);
  const { result, rerender } = renderHook(() => useUpdateNotificationWebhook(), {
    wrapper: ({ children }: { children: ReactNode }) =>
      createElement(QueryClientProvider, { client }, children),
  });
  ...
  act(() => {
    completion = result.current
      .mutateAsync({ id: "original-id", etag: '"original"', name: "updated" })
      .catch((error) => error);
  });
...
it("preserves the invocation body and observed validator while paused", async () => {
  const client = new QueryClient();
  ...
  onlineManager.setOnline(false);
  const input = { id: "row-one", etag: '"original"', name: "original name", enabled: false };
  act(() => {
    result.current.mutate(input);
  });
  await waitFor(() => expect(result.current.isPaused).toBe(true));
  input.etag = '"replacement"';
