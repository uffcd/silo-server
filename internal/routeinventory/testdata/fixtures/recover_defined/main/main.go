...
import (
	...
	"github.com/Silo-Server/silo-server/internal/routeinventory/testdata/fixtures/recover_defined/listener"
)

func extra(w http.ResponseWriter, r *http.Request) {}

type routerDefined chi.Router

func late(h http.Handler) {
	if r, ok := h.(routerDefined); ok {
		r.Get("/hidden", extra)
	}
}
