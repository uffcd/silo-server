...
func TestRequiredInviteCodeTopUpsPairingCannotChange(t *testing.T) {
	catalogs, err := Load()
	if err != nil {
		t.Fatal(err)
	}
	selected, err := InviteCodeTopUpsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range RequiredInviteCodeTopUpsScenarios {
					...
					for ri := range c.Rows {
						...
						for i := range row.Scenarios {
							...
							case "sequence":
								s.Then = []Step{{}}
							case "original body":
								s.Request.Body = json.RawMessage(`{"additional_uses":99}`)
							case "translated body":
								s.V2Expectation.Request.Body = json.RawMessage(`{"additional_uses":99}`)
							case "status":
								s.V2Expectation.Expect.Status = 403
							}
							break
						}
					}
				}
				if _, err := InviteCodeTopUpsAcceptance(changed); err == nil || !strings.Contains(err.Error(), id) {
