...
func TestRequiredAdminInvitationEmailConflictsPairingCannotChange(t *testing.T) {
	catalogs, err := Load()
	if err != nil {
		t.Fatal(err)
	}
	selected, err := AdminInvitationEmailConflictsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range RequiredAdminInvitationEmailConflictsScenarios {
		for _, failure := range []string{"missing case", "duplicate case", "missing pair", "wrong operation", "wrong method", "wrong route", "changed bearer", "original request", "principal", "pair principal", "requirements", "settings", "sequence", "status", "original body", "translated body"} {
			t.Run(id+"/"+failure, func(t *testing.T) {
				...
				if _, err := AdminInvitationEmailConflictsAcceptance(changed); err == nil || !strings.Contains(err.Error(), id) {
