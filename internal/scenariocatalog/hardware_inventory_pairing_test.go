...
func TestHardwareInventorySelection(t *testing.T) {
	catalogs, err := Load()
	if err != nil {
		t.Fatal(err)
	}
	selected, err := HardwareInventoryAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range RequiredHardwareInventoryScenarios {
		for _, failure := range []string{"missing", "duplicate", "pair", "authority", "repeat", "requirement", "operation", "body", "sequence"} {
			t.Run(id+"/"+failure, func(t *testing.T) {
				...
				if _, err := HardwareInventoryAcceptance(changed); err == nil || !strings.Contains(err.Error(), id) {
