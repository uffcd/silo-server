...
func TestRequiredInviteCodeUpdateRefusalsPairingCannotChange(t *testing.T) {
	catalogs, err := Load()
	if err != nil {
		t.Fatal(err)
	}
	selected, err := InviteCodeUpdateRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range RequiredInviteCodeUpdateRefusalsScenarios {
		for _, failure := range []string{"missing case", "duplicate case", "missing pair", "wrong operation", "wrong method", "wrong route", "changed bearer", "original request", "principal", "pair principal", "requirements", "settings", "sequence", "status", "original body", "translated body"} {
			t.Run(id+"/"+failure, func(t *testing.T) {
					...
					for ri := range c.Rows {
						...
						for i := range row.Scenarios {
							...
							case "sequence":
								s.Then = []Step{{}}
							case "original body":
								s.Request.Body = json.RawMessage(`{"label":"changed"}`)
							case "translated body":
								s.V2Expectation.Request.Body = json.RawMessage(`{"label":"changed"}`)
							case "status":
				...
				if _, err := InviteCodeUpdateRefusalsAcceptance(changed); err == nil || !strings.Contains(err.Error(), id) {
