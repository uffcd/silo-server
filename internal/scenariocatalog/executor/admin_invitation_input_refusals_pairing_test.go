...
func TestRequiredAdminInvitationInputRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-admin-invitation-input-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AdminInvitationInputRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAdminInvitationInputRefusalsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 4 || effects != 8 {
		t.Errorf("paired administrator invitation create refusal evidence %dHTTP/%dPG, want4/8", requests, effects)
	}
