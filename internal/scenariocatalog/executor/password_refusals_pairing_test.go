...
func TestRequiredPasswordRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-password-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.PasswordRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/login-session/device-request rows changed during password refusals read")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredPasswordRefusalsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 10 || effects != 20 {
		t.Errorf("paired password refusals evidence %dHTTP/%dPG, want10/20", requests, effects)
	}
