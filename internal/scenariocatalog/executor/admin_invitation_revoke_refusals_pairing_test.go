...
func TestRequiredAdminInvitationRevokeRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-admin-invitation-revoke-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AdminInvitationRevokeRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil {
		t.Fatal("connect administrator invitation revoke refusal scratch database")
	}
	defer pool.Close()
	...
	t.Log("administrator invitation revoke refusal pre-constructor guards passed")
	e := New(t)
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/session/device-request/invitation/code rows changed during invitation revoke refusal")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAdminInvitationRevokeRefusalsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 6 || effects != 12 {
		t.Errorf("paired administrator invitation revoke refusal evidence %dHTTP/%dPG, want6/12", requests, effects)
	}
	t.Logf("administrator invitation revoke refusals: %d results, %d HTTP, %d snapshots, %d table observations", len(results), requests, effects, effects*8)
