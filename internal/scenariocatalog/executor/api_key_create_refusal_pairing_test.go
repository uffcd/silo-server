...
func TestRequiredAPIKeyCreateRefusalAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-api-key-create-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.APIKeyCreateRefusalAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
				...
				for _, transport := range []string{"v1", "v2"} {
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						if len(before) != len(after) {
							t.Error("unexpected API-key row count after creation refusal")
						}
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("API-key rows changed during creation refusal")
							}
						}
					})
				}
