...
func TestRequiredMeImpersonationAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-me-impersonation for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.MeImpersonationAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
			...
			for _, s := range row.Scenarios {
				for _, transport := range []string{"v1", "v2"} {
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						// The original header uses the lazily issued real impersonation
						// fixture. Complete that setup before observing the account read.
						if _, err := e.substitute("${impersonation_token}"); err != nil {
							t.Fatal(err)
						}
						before := snapshot()
						if len(before) != 8 {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/login-session/device-request/invitation/invite-code rows changed during impersonated account read")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredMeImpersonationScenarios); err != nil {
		t.Error(err)
	}
	if requests != 2 || effects != 4 {
		t.Errorf("paired impersonated account read evidence %dHTTP/%dPG, want2/4", requests, effects)
	}
