...
func TestRequiredDeviceApproveRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-device-approve-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.DeviceApproveRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/login-session/device-request rows changed during device approve refusals read")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredDeviceApproveRefusalsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 8 || effects != 16 {
		t.Errorf("paired device approve refusals evidence %dHTTP/%dPG, want8/16", requests, effects)
	}
