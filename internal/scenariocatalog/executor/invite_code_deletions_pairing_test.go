...
func TestRequiredInviteCodeDeletionsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-invite-code-deletions for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.InviteCodeDeletionsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
			...
			for _, s := range row.Scenarios {
				for _, transport := range []string{"v1", "v2"} {
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						after := snapshot()
						target := e.fixtures["invite_code_id"]
						if s.ID == "codes_delete.shape" {
							target = e.fixtures["invite_code_disabled_id"]
						}
						assertInviteCodeDeletion(t, target, before["invite_codes"], after["invite_codes"])
						...
						for id, want := range before {
							...
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/session/device-request/invitation/code rows changed during invite-code deletion refusal")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredInviteCodeDeletionsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 8 || effects != 12 {
		t.Errorf("paired administrator invite-code refusal evidence %dHTTP/%dPG, want8/12", requests, effects)
	}
...
func assertInviteCodeDeletion(t *testing.T, target string, before, after json.RawMessage) {
	t.Helper()
	...
	for _, want := range previous {
		...
		if remaining >= len(current) {
			t.Fatal("unexpected missing surviving code")
		}
		got := current[remaining]
		remaining++
		if len(want) != len(got) {
			t.Error("surviving code column count changed")
		}
		for key, value := range want {
			if !bytes.Equal(value, got[key]) {
				t.Errorf("unexpected surviving code field change: %s", key)
			}
		}
	}
