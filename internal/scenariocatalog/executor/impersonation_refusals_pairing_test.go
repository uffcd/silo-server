...
func TestRequiredImpersonationRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-impersonation-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.ImpersonationRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/login-session/device-request rows changed during impersonation refusals read")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredImpersonationRefusalsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 4 || effects != 8 {
		t.Errorf("paired impersonation refusals evidence %dHTTP/%dPG, want4/8", requests, effects)
	}
