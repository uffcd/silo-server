...
const invitationAcceptedPassword = "fixture-invitee-password"
const invitationAcceptedEmail = "fixture-invitee@silo.example.test"

func invitationTokenCreates(id string) bool {
	return id == "inv_accept.ok" || id == "inv_accept.meaning" || id == "inv_accept.shape" || id == "inv_accept.single_use"
}
func invitationTokenChangedTable(id, table string) bool {
	return invitationTokenCreates(id) && (table == "users" || table == "profiles" || table == "sessions" || table == "invitations")
}

func assertInvitationTokenEffects(t *testing.T, e *Env, id string, resp response, before, after map[string]json.RawMessage, dbStart, dbEnd, appStart, appEnd time.Time) {
	t.Helper()
	if !invitationTokenCreates(id) {
		return
	}
	...
	wantUser["id"] = user["id"]
	wantUser["username"] = encode(invitationAcceptedEmail)
	wantUser["email"] = encode(invitationAcceptedEmail)
	if err := bcrypt.CompareHashAndPassword([]byte(text(user["password_hash"])), []byte(invitationAcceptedPassword)); err != nil {
