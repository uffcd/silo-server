...
func TestRequiredAccountMeRefusalAcceptance(t *testing.T) {
	...
	selected, err := scenariocatalog.AccountMeRefusalAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Log("account refusal pre-constructor guards passed")
	e := New(t)
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key rows changed during auth refusal")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAccountMeRefusalScenarios); err != nil {
		t.Error(err)
	}
	if requests != 4 || effects != 8 {
		t.Errorf("paired account refusal evidence %dHTTP/%dPG, want4/8", requests, effects)
	}
	t.Logf("account refusals: %d results, %d HTTP, %d snapshots, %d table observations", len(results), requests, effects, effects*3)
