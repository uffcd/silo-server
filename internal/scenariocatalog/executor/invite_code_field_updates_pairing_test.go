...
func TestRequiredInviteCodeFieldUpdatesAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-invite-code-field-updates for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.InviteCodeFieldUpdatesAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
			...
			for _, s := range row.Scenarios {
				for _, transport := range []string{"v1", "v2"} {
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						assertInviteCodeFieldUpdate(t, s.ID, e.fixtures["invite_code_id"], before["invite_codes"], after["invite_codes"], started, finished)
						...
						for id, want := range before {
							...
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/session/device-request/invitation/code rows changed during invite-code field update refusal")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredInviteCodeFieldUpdatesScenarios); err != nil {
		t.Error(err)
	}
...
// Compare every code column, allowing only the reserved field and a bounded update time.
func assertInviteCodeFieldUpdate(t *testing.T, scenario, target string, before, after json.RawMessage, started, finished time.Time) {
	t.Helper()
	...
	for i, want := range previous {
		...
		if string(want["id"]) == target {
			...
			case "codes_update.ok":
				want["label"] = json.RawMessage(`"renamed"`)
			case "codes_update.shape":
				want["label"] = json.RawMessage(`"x"`)
			case "codes_update.disable":
				want["enabled"] = json.RawMessage(`false`)
			default:
				t.Fatal("unsupported field-update effect")
			}
