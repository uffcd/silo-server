...
func TestRequiredApprovalStateRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-approval-state-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.ApprovalStateRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/login-session/device-request rows changed during approval state refusals read")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredApprovalStateRefusalsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 6 || effects != 12 {
		t.Errorf("paired approval state refusals evidence %dHTTP/%dPG, want6/12", requests, effects)
	}
