...
func TestRequiredAuthProvidersAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-auth-providers for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AuthProvidersAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAuthProvidersScenarios); err != nil {
		t.Error(err)
	}
	if requests != 10 || effects != 20 {
		t.Errorf("paired auth providers evidence %dHTTP/%dPG, want10/20", requests, effects)
	}
