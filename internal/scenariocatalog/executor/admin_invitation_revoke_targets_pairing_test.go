...
func TestRequiredAdminInvitationRevokeTargetsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-admin-invitation-revoke-targets for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AdminInvitationRevokeTargetsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil {
		t.Fatal("connect administrator invitation revoke target refusal scratch database")
	}
	defer pool.Close()
	...
	t.Log("administrator invitation revoke target refusal pre-constructor guards passed")
	e := New(t)
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/session/device-request/invitation/code rows changed during invitation revoke target refusal")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAdminInvitationRevokeTargetsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 4 || effects != 8 {
		t.Errorf("paired administrator invitation revoke target refusal evidence %dHTTP/%dPG, want4/8", requests, effects)
	}
	t.Logf("administrator invitation revoke target refusals: %d results, %d HTTP, %d snapshots, %d table observations", len(results), requests, effects, effects*8)
