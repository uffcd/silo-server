...
func TestRequiredInviteCodeListReadsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-invite-code-list-reads for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.InviteCodeListReadsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/session/device-request/invitation/code rows changed during invite-code list read refusal")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredInviteCodeListReadsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 8 || effects != 16 {
		t.Errorf("paired administrator invite-code refusal evidence %dHTTP/%dPG, want8/16", requests, effects)
	}
	t.Logf("administrator invite-code reads: %d results, %d HTTP, %d snapshots, %d table observations", len(results), requests, effects, effects*8)
