...
func TestRequiredLoginSessionsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-login-sessions for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.LoginSessionsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
				...
				for _, transport := range []string{"v1", "v2"} {
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						snapshot := func() map[string]json.RawMessage {
							...
							effects++
							var raw []byte
							if err := e.pool.QueryRow(e.ctx, `SELECT jsonb_build_object('users',(SELECT jsonb_agg(to_jsonb(u) ORDER BY id) FROM users u),'profiles',(SELECT jsonb_agg(to_jsonb(p) ORDER BY id) FROM user_profiles p),'keys',(SELECT jsonb_agg(to_jsonb(k) ORDER BY id) FROM api_keys k),'settings',(SELECT jsonb_agg(to_jsonb(s) ORDER BY key) FROM server_settings s),'sessions',(SELECT jsonb_agg(to_jsonb(a) ORDER BY id) FROM auth_sessions a))`).Scan(&raw); err != nil {
								t.Fatal(err)
							}
						...
						before := snapshot()
						if len(before) != 5 {
							t.Fatal("snapshot must contain five full tables")
						}
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/login-session rows changed during login sessions read")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredLoginSessionsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 12 || effects != 24 {
		t.Errorf("paired login sessions evidence %dHTTP/%dPG, want12/24", requests, effects)
	}
