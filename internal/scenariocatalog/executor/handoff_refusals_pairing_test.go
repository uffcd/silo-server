...
func TestRequiredHandoffRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-handoff-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.HandoffRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/login-session/device-request rows changed during handoff refusals read")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredHandoffRefusalsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 10 || effects != 20 {
		t.Errorf("paired handoff refusals evidence %dHTTP/%dPG, want10/20", requests, effects)
	}
