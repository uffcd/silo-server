...
func TestRequiredAPIKeyScopesAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-api-key-scopes for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.APIKeyScopesAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
				...
				for _, transport := range []string{"v1", "v2"} {
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						if len(before) != len(after) {
							t.Error("unexpected API-key row count after scope discovery")
						}
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("API-key rows changed during scope discovery")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAPIKeyScopesScenarios); err != nil {
		t.Error(err)
	}
	if requests != 12 || effects != 24 {
		t.Errorf("paired scope discovery evidence %dHTTP/%dPG, want12/24", requests, effects)
	}
