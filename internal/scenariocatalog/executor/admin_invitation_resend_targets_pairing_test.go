...
func TestRequiredAdminInvitationResendTargetsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-admin-invitation-resend-targets for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AdminInvitationResendTargetsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAdminInvitationResendTargetsScenarios); err != nil {
		t.Error(err)
	}
