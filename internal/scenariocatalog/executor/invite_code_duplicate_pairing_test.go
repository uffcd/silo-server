...
func TestRequiredInviteCodeDuplicateAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-invite-code-duplicate for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.InviteCodeDuplicateAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	var results []Result
	requests, effects, sequenceObservations := 0, 0, 0
				...
				for _, transport := range []string{"v1", "v2"} {
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						sequence := func() int64 {
							...
							var value int64
							var called bool
							if err := e.pool.QueryRow(e.ctx, `SELECT last_value, is_called FROM invite_codes_id_seq`).Scan(&value, &called); err != nil {
								t.Fatal(err)
							}
							if !called {
								t.Fatal("seeded invite-code sequence must have been called")
							}
							return value
						}
						...
						after := snapshot()
						if sequenceAfter := sequence(); sequenceAfter != sequenceBefore+1 {
							t.Errorf("duplicate insert sequence advance = %d, want 1", sequenceAfter-sequenceBefore)
						}
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/session/device-request/invitation/code rows changed during invite-code duplicate-code refusal")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredInviteCodeDuplicateScenarios); err != nil {
		t.Error(err)
	}
	if sequenceObservations != 4 {
		t.Errorf("sequence observations = %d, want 4", sequenceObservations)
	}
	t.Logf("duplicate inserts: %d sequence observations; each transport requires exactly one allocation", sequenceObservations)
	if requests != 2 || effects != 4 {
		t.Errorf("paired administrator invite-code refusal evidence %dHTTP/%dPG, want2/4", requests, effects)
	}
