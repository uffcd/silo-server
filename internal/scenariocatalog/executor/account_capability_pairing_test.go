...
func TestRequiredAccountCapabilityAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-account-capability for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AccountCapabilityAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key rows changed during capability read")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAccountCapabilityScenarios); err != nil {
		t.Error(err)
	}
	if requests != 14 || effects != 28 {
		t.Errorf("paired account capability evidence %dHTTP/%dPG, want14/28", requests, effects)
	}
