...
func TestRequiredAdminInvitationRoleRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-admin-invitation-role-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AdminInvitationRoleRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAdminInvitationRoleRefusalsScenarios); err != nil {
		t.Error(err)
	}
