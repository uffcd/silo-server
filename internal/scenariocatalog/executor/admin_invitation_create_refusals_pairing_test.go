...
func TestRequiredAdminInvitationCreateRefusalsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-admin-invitation-create-refusals for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AdminInvitationCreateRefusalsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil {
		t.Fatal("connect administrator invitation create refusal scratch database")
	}
	defer pool.Close()
	...
	t.Log("administrator invitation create refusal pre-constructor guards passed")
	e := New(t)
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/session/device-request/invitation/code rows changed during invitation create refusal")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAdminInvitationCreateRefusalsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 6 || effects != 12 {
		t.Errorf("paired administrator invitation create refusal evidence %dHTTP/%dPG, want6/12", requests, effects)
	}
	t.Logf("administrator invitation create refusals: %d results, %d HTTP, %d snapshots, %d table observations", len(results), requests, effects, effects*8)
