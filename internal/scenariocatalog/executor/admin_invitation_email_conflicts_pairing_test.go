...
func TestRequiredAdminInvitationEmailConflictsAcceptance(t *testing.T) {
	if os.Getenv("SILO_SCENARIO_REQUIRED") != "1" {
		t.Skip("run make test-scenario-admin-invitation-email-conflicts for required paired acceptance")
	}
	...
	selected, err := scenariocatalog.AdminInvitationEmailConflictsAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil {
		t.Fatal("connect administrator invitation email conflict scratch database")
	}
	defer pool.Close()
	...
	t.Log("administrator invitation email conflict pre-constructor guards passed")
	e := New(t)
					...
					t.Run(s.ID+"/"+transport, func(t *testing.T) {
						...
						for id, want := range before {
							if !bytes.Equal(want, after[id]) {
								t.Error("account/profile/API-key/settings/session/device-request/invitation/code rows changed during invitation email conflict")
							}
						}
					})
				}
	...
	if err := requiredPairedResults(results, scenariocatalog.RequiredAdminInvitationEmailConflictsScenarios); err != nil {
		t.Error(err)
	}
	if requests != 4 || effects != 8 {
		t.Errorf("paired administrator invitation email conflict evidence %dHTTP/%dPG, want4/8", requests, effects)
	}
	t.Logf("administrator invitation email conflicts: %d results, %d HTTP, %d snapshots, %d table observations", len(results), requests, effects, effects*8)
