...
func TestResourcesHandoffImpersonationSelection(t *testing.T) {
	catalogs, err := Load()
	if err != nil {
		t.Fatal(err)
	}
	selected, err := ResourcesHandoffImpersonationAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range RequiredResourcesHandoffImpersonationScenarios {
		for _, failure := range []string{"missing", "duplicate", "pair", "authority", "repeat", "requirement", "operation", "body", "sequence"} {
			t.Run(id+"/"+failure, func(t *testing.T) {
				...
				if _, err := ResourcesHandoffImpersonationAcceptance(changed); err == nil || !strings.Contains(err.Error(), id) {
