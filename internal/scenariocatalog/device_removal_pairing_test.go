...
func TestDeviceRemovalSelection(t *testing.T) {
	catalogs, err := Load()
	if err != nil {
		t.Fatal(err)
	}
	selected, err := DeviceRemovalAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range RequiredDeviceRemovalScenarios {
		for _, failure := range []string{"missing", "duplicate", "pair", "authority", "repeat", "requirement", "operation", "body", "sequence"} {
			t.Run(id+"/"+failure, func(t *testing.T) {
				...
				if _, err := DeviceRemovalAcceptance(changed); err == nil || !strings.Contains(err.Error(), id) {
