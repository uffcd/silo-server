...
func TestRequiredAccountMeRefusalPairingCannotChange(t *testing.T) {
	catalogs, err := Load()
	if err != nil {
		t.Fatal(err)
	}
	selected, err := AccountMeRefusalAcceptance(catalogs)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range RequiredAccountMeRefusalScenarios {
		for _, failure := range []string{"missing case", "duplicate case", "missing pair", "wrong operation", "wrong method", "wrong route", "changed bearer", "original request", "principal", "pair principal", "requirements", "settings", "sequence", "status"} {
			t.Run(id+"/"+failure, func(t *testing.T) {
				...
				for _, c := range changed {
					for ri := range c.Rows {
						...
						for i := range row.Scenarios {
							...
							case "changed bearer":
								s.V2Expectation.Request.Headers = map[string]*string{accountMeAuthorizationHeader: new("Bearer replacement")}
							case "original request":
								s.Request.Repeat = 2
							case "principal":
								s.Principal.Class = "member"
							case "pair principal":
								s.V2Expectation.Principal = &Principal{Class: accountMePublicPrincipal}
							case "requirements":
				...
				if _, err := AccountMeRefusalAcceptance(changed); err == nil || !strings.Contains(err.Error(), id) {
